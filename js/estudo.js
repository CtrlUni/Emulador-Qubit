...
function youTubePlayerDisplayFixedInfos() {
    ...
    if (youTubePlayerActive()) {
        document.getElementById('YouTube-player-fixed-infos').innerHTML = (
            'Código embutido: <textarea readonly>' + youTubePlayer.getVideoEmbedCode() + '</textarea>'
        );
    }
}
...
function youTubePlayerDisplayInfos() {
    ...
    if (youTubePlayerActive()) {
        ...
        document.getElementById('YouTube-player-infos').innerHTML = (
            indicatorDisplay
                + 'Link: <a class="url" href="' + url + '">' + url + '</a><br>'
                + 'Qualidade: <strong>' + youTubePlayer.getPlaybackQuality() + '</strong>'
                + ' &mdash; Qualidade avaliada: <strong>' + youTubePlayer.getAvailableQualityLevels() + '</strong><br>'
                + 'Estado <strong>' + state + '</strong>: <strong>' + youTubePlayerStateValueToDescription(state) + '</strong><br>'
                + 'Carga: <strong>' + (fraction*100).toFixed(1) + '</strong>%<br>'
                + 'Duração: <strong>' + current.toFixed(2) + '</strong>/<strong>' + duration.toFixed(2) + '</strong>s = <strong>' + currentPercent.toFixed(2) + '</strong>%<br>'
